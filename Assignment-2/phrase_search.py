...
def tokenize(document):
    ...
    """
    hey = document.lower() 
    return re.findall(r"[A-Za-z'-]+", hey)
    pass
...
def create_positional_index(tokens):
    ...
    for docID, lists in enumerate(tokens):
        for element in set(lists):
            pos[element].append([docID] + [index for index, ele in enumerate(lists) if ele == element])
    return pos
...
def phrase_intersect(list1, list2):
    ...
    while i < len(list1) and j < len(list2):
        if list1[i][0] == list2[j][0]:
            final = lists(list1[i][1:],list2[j][1:])
            if len(final) != 0:
                result.append(([list2[j][0]]+final))
            i = i+1
            j = j+1
...
def search(index, query):
    ...
    i = 1
    while terms[i:]:
        inter = phrase_intersect(inter,index[terms[i]])
        i += 1
    
    return [r[0] for r in inter]
...
def find_top_bigrams(terms, n):
    ...
    for element in terms:
        c.update(list_to_bigrams(element))
